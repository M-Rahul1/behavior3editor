...
gulp.task('_app_html', function() {
  return gulp.src(app_html)
             ...
             .pipe(replace('[BUILD_VERSION]', build_version))
             .pipe(replace('[BUILD_DATE]', build_date))
             .pipe(templateCache('templates.min.js', {standalone:true, root:'/templates/'}))
             .pipe(gulp.dest('build/js'))
             .pipe(connect.reload())
});
...
// TASKS (LIVE RELOAD) ========================================================
gulp.task('_connect', async function() {
  connect.server({
    livereload: true,
    root: 'build',
    host: '::',
...
gulp.task('_watch', async function() {
...
gulp.task('serve', gulp.series('_vendor', '_preload', '_app_dev', '_connect', '_watch'));

// TASKS (NODE WEBKIT) ========================================================
gulp.task('_electron', gulp.series('build', function() {
  return packager({
    dir       : 'build',
    out       : '.temp-dist',
    name      : project.name,
